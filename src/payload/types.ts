...
export interface PayloadDescriptorBase<
  Kind extends string,
...
export interface PayloadStringDescriptor<
  Required extends boolean = true,
  Default extends string | null | undefined = undefined,
> extends PayloadDescriptorBase<"string", Required, Default> {}

export interface PayloadNumberDescriptor<
  Required extends boolean = true,
  Default extends number | null | undefined = undefined,
> extends PayloadDescriptorBase<"number", Required, Default> {}

export interface PayloadBooleanDescriptor<
  Required extends boolean = true,
  Default extends boolean | null | undefined = undefined,
> extends PayloadDescriptorBase<"boolean", Required, Default> {}

export type PayloadObjectDefinition = PayloadDefinition | PayloadSchema<any, any>;

export interface PayloadObjectDescriptor<
  Definition extends PayloadObjectDefinition,
  Required extends boolean = true,
  Default = undefined,
> extends PayloadDescriptorBase<"object", Required, Default> {
  readonly definition: Definition;
}

export interface PayloadArrayDescriptor<
  Item extends PayloadValueDefinition,
  Required extends boolean = true,
  Default = undefined,
> extends PayloadDescriptorBase<"array", Required, Default> {
  readonly item: Item;
}

export type PayloadPrimitiveDescriptor =
  | PayloadStringDescriptor<any, any>
  | PayloadNumberDescriptor<any, any>
  | PayloadBooleanDescriptor<any, any>;

export type PayloadDescriptor =
  | PayloadPrimitiveDescriptor
  | PayloadObjectDescriptor<any, any, any>
  | PayloadArrayDescriptor<any, any, any>;

export type PayloadValueDefinition =
  PayloadDescriptor | PayloadParser<any, any> | PayloadSchema<any, any>;
...
type ApplyRawOptions<Raw, Value extends PayloadValueDefinition> =
  Value extends PayloadDescriptorBase<any, false, any> ? Raw | null | "" : Raw;

type ApplyParsedOptions<Parsed, Value extends PayloadValueDefinition> =
  Value extends PayloadDescriptorBase<any, false, infer Default> ? Parsed | Default : Parsed;

type OptionalRawKeys<Definition extends PayloadDefinition> = {
  [Key in keyof Definition]-?: Definition[Key] extends PayloadDescriptorBase<any, false, any>
    ? Key
    : never;
}[keyof Definition];

type RequiredRawKeys<Definition extends PayloadDefinition> = Exclude<
  keyof Definition,
  OptionalRawKeys<Definition>
>;

type InferRawObject<
  ObjectDefinition extends PayloadObjectDefinition,
  ParentOptions extends PayloadSchemaOptions,
> =
  ObjectDefinition extends PayloadSchema<infer Definition, infer Options>
    ? InferRawDefinition<Definition, Options>
    : ObjectDefinition extends PayloadDefinition
      ? InferRawDefinition<ObjectDefinition, ParentOptions>
      : never;

type InferParsedObject<ObjectDefinition extends PayloadObjectDefinition> =
  ObjectDefinition extends PayloadSchema<infer Definition, any>
    ? InferParsedDefinition<Definition>
    : ObjectDefinition extends PayloadDefinition
      ? InferParsedDefinition<ObjectDefinition>
      : never;

export type InferRawValue<
  Value extends PayloadValueDefinition,
  Options extends PayloadSchemaOptions,
> =
  Value extends PayloadSchema<infer Definition, infer SchemaOptions>
    ? InferRawDefinition<Definition, SchemaOptions>
    : Value extends PayloadParser<infer Raw, any>
      ? Raw
      : Value extends PayloadStringDescriptor<any, any>
        ? ApplyRawOptions<string, Value>
        : Value extends PayloadNumberDescriptor<any, any>
          ? ApplyRawOptions<PrimitivesFromString<Options> extends true ? string : number, Value>
          : Value extends PayloadBooleanDescriptor<any, any>
            ? ApplyRawOptions<PrimitivesFromString<Options> extends true ? string : boolean, Value>
            : Value extends PayloadObjectDescriptor<infer Definition, any, any>
              ? ApplyRawOptions<InferRawObject<Definition, Options>, Value>
              : Value extends PayloadArrayDescriptor<infer Item, any, any>
                ? ApplyRawOptions<InferRawValue<Item, Options>[], Value>
                : never;

export type InferParsedValue<Value extends PayloadValueDefinition> =
  Value extends PayloadSchema<infer Definition, any>
    ? InferParsedDefinition<Definition>
    : Value extends PayloadParser<any, infer Parsed>
      ? Parsed
      : Value extends PayloadStringDescriptor<any, any>
        ? ApplyParsedOptions<string, Value>
        : Value extends PayloadNumberDescriptor<any, any>
          ? ApplyParsedOptions<number, Value>
          : Value extends PayloadBooleanDescriptor<any, any>
            ? ApplyParsedOptions<boolean, Value>
            : Value extends PayloadObjectDescriptor<infer Definition, any, any>
              ? ApplyParsedOptions<InferParsedObject<Definition>, Value>
              : Value extends PayloadArrayDescriptor<infer Item, any, any>
                ? ApplyParsedOptions<InferParsedValue<Item>[], Value>
                : never;
 ...
 */
export type InferRawDefinition<
